...
impl HttpReranker {
    ...
    fn parse_response(&self, response: serde_json::Value) -> Result<Vec<RerankResult>> {
        ...
        for result in results {
            ...
            // BGE / MS-MARCO / Qwen3 cross-encoders served via llama.cpp's
            // /v1/rerank emit raw classifier logits. Apply sigmoid so the
            // engine's `min_rerank_score` floor (tuned for [0,1] scores)
            // behaves the same across cloud and local rerankers. Sigmoid is
            // monotonic, so ordering is preserved.
            let score = if self.config.sigmoid_normalize {
                1.0 / (1.0 + (-raw).exp())
            } else {
                raw
            };
