...
pub struct RerankConfig {
    ...
    /// Apply `sigmoid` to scores after parsing.
    ///
    /// Required for cross-encoders served via llama.cpp's `/v1/rerank`
    /// (BGE family, most MS-MARCO cross-encoders, Qwen3 rerankers) that
    /// emit raw classifier logits in roughly `[-15, +15]`. Cloud APIs
    /// (Jina, Cohere, Aliyun) already return `[0, 1]` so this stays `false`.
    ///
    /// Sigmoid is monotonic — ordering is unchanged, only the absolute
    /// scale shifts to `(0, 1)` so the engine-wide `min_rerank_score`
    /// floor (tuned for normalized scores) behaves consistently across
    /// rerankers.
    pub sigmoid_normalize: bool,
}
...
impl RerankConfig {
    ...
    /// Enable sigmoid normalization of raw classifier logits.
    pub fn with_sigmoid_normalize(mut self, enable: bool) -> Self {
        self.sigmoid_normalize = enable;
        self
    }
}
