...
/// Reranker for cross-encoder relevance scoring).
...
pub enum ModelType {
    ...
    Multimodal,
    /// Cross-encoder reranker (e.g. jina-reranker-v3) reachable via
    /// `/v1/rerank`. Selected by workspaces with
    /// `reranker_strategy = "semantic"`.
    Reranker,
}
...
impl ModelsConfig {
    ...
    /// Get all reranker (cross-encoder) models across all providers.
    pub fn all_reranker_models(&self) -> Vec<(&ProviderConfig, &ModelCard)> {
        self.providers
            .iter()
            .filter(|p| p.enabled)
            .flat_map(|p| {
                p.models
                    .iter()
                    .filter(|m| matches!(m.model_type, ModelType::Reranker))
                    .map(move |m| (p, m))
            })
            .collect()
    }
